...
def _collect_entries(log) -> List[LogEntry]:
    ...
    for entry_id, tag, timestamp, payload in log.iter_entries_with_id():
        entries.append(
            LogEntry(tag=tag, payload=payload, index=entry_id, timestamp=timestamp or None)
        )
    return entries
...
async def summarize_conversation() -> bool:
    ...
    remaining_entries = [
        LogEntry(tag=tag, payload=payload, index=entry_id, timestamp=timestamp or None)
        for entry_id, tag, timestamp, payload in conversation_log.iter_entries_after(cutoff_index)
    ]
