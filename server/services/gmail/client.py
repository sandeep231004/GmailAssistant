...
_CONNECTION_LOGGED_LOCK = threading.Lock()
_CONNECTION_LOGGED: set[str] = set()
...
def set_active_gmail_user_id(user_id: Optional[str]) -> None:
    """Public setter used by request handlers to keep Gmail user context in sync."""
...
def _clear_cached_profile(user_id: Optional[str] = None) -> None:
    ...
    if user_id:
        with _CONNECTION_LOGGED_LOCK:
            _CONNECTION_LOGGED.discard(_normalized(user_id))
    else:
        with _CONNECTION_LOGGED_LOCK:
            _CONNECTION_LOGGED.clear()
    ...
    try:
        ...
        if allow_multiple:
            try:
                ...
                logger.warning(
                    "Composio SDK does not support allow_multiple; retrying without the flag",
...
def fetch_status(payload: GmailStatusPayload) -> JSONResponse:
    ...
    try:
        ...
        if connected and user_id:
            _maybe_log_connected(user_id, email)
...
def _maybe_log_connected(user_id: str, email: Optional[str]) -> None:
    normalized = _normalized(user_id)
    if not normalized:
        return
    with _CONNECTION_LOGGED_LOCK:
        if normalized in _CONNECTION_LOGGED:
            return
        _CONNECTION_LOGGED.add(normalized)

    try:
        from ..conversation import get_conversation_log

        log = get_conversation_log()
        detail = email or "unknown email"
        log.record_agent_message(
            f"Gmail connection active for user_id={normalized} ({detail})."
        )
    except Exception:
        pass
